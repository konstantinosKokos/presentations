...
\newcommand{\alertstructat}[3][]{\alt<#2>{\struct[#1]{#3}}{\struct[\#1]{#3}}}
...
\newcommand{\predc}{\dep{pc}}
\newcommand{\body}{\dep{body}}
\newcommand{\verc}{\dep{vc}}
...
\quad\light{Variations on a Theme}}\smaller
	...
	consider instead; die :: $\ty{\dxbox[\dmod](np \bs np) / \dxdia[\body](\dxdia[\su] np \bs ssub) \mathbin{\&} \dxbox[\dmod](np \bs np) / \dxdia[\body](\ddia\dbox\dxdia[\obj] np \bs ssub)}$
	}}{
	\smaller
	\alt<3->{
		\[
...
\quad\light{Hypothetical Reasoning 2: Horizontal Movement}}\smaller
	die :: $\ty{\dxbox[\dmod](np \bs np) / \dxdia[\body](\ddia\dbox\dxdia[\obj]np \bs ssub)}$
	\vfill

	\[
		\infer*{}{
			\infer[\dxdia_I]{\struct[\body]{(\struct[\su]{\w{vrouwen}} \cdot \w{haten})} \vdash \ty{\dxdia[\body](\ddia\dbox\dxdia[\obj]np \bs ssub )}}{
				\infer[\bs_I]{(\struct[\su]{\w{vrouwen}} \cdot \w{haten}) \vdash \ty{\ddia\dbox\dxdia[\obj]np \bs ssub}}{
					\infer[\dxdia_E]{(\ty{\ddia\dbox\dxdia[\obj]np} \cdot (\struct[\su]{\w{vrouwen}} \cdot \w{haten})) \vdash \ty{ssub}}{
						\infer[\Ax]{\ty{\ddia\dbox\dxdia[\obj]np} \vdash \ty{\ddia\dbox\dxdia[\obj]np}}{}
						&
						\infer[\alert{\rulestyle{C}^l_{\ddia}}]{(\struct{\ty{\dbox\dxdia[\obj]np}} \cdot (\struct[\su]{\w{vrouwen}} \cdot \w{haten})) \vdash \ty{ssub}}{
...
\quad\light{Hypothetical Reasoning 4: Vertical Movement}}\smaller
	\alt<3->{
	Structural blockades backfiring...
	
	\vfill
	\hfil Perhaps:
		\[
			\infer[\rulestyle{X}]{\Gamma\subst{\struct[μ]{\Delta} \cdot \struct[ν]{\ty{A}}} \vdash \ty{B}}{\Gamma\subst{\struct[μ]{\Delta \cdot \struct[ν]{\ty{A}}}} \vdash \ty{B}}
		\]
	}{
	\lightat{2}{
	\[
		\infer[/_E]{}{
			\infer*{(\w{op} \cdot \w{wie}) \vdash \ty{whq\bs\dxdia[\body](sv1\bs\alert{???}\dxdia[\predc]{prp})}}{}
			&
			\hspace{-6em}
			\infer*[\alert{?}]{\struct[\body]{((\w{heb} \cdot \struct[\su]{\w{je}}) \cdot \w{gestemd})} \vdash \ty{\dxdia[\body]sv1/\alert{???}\dxdia[\predc] prp}}{
				\hspace{-2em}
				\infer[\dxdia_I]{\alertstructat[\body]{2-}{((\w{heb} \cdot \struct[\su]{\w{je}}) \cdot (\w{gestemd} \cdot \ty{\dxdia[\predc] prp}))} \vdash \ty{\dxdia[\body]sv1}}{
					\infer[/_E]{((\w{heb} \cdot \struct[\su]{\w{je}}) \cdot (\w{gestemd} \cdot \ty{\dxdia[\predc] prp})) \vdash \ty{sv1}}{
						\infer[/_E]{(\w{heb} \cdot \struct[\su]{\w{je}}) \vdash \ty{sv1/\dxdia[\verc]ppart}}{
							\infer[\Lex]{\ty{(sv1/\dxdia[\verc]ppart)/\dxdia[\su]np) }}{\w{heb}}
							&
							\infer[\dxdia_I]{\struct[\su]{\w{je}} \vdash \ty{\dxdia[\su]np}}{
								\infer[\Lex]{\ty{np}}{\w{je}}
							}
						}
						&
						\infer[/_E]{(\w{gestemd} \cdot \ty{\dxdia[\predc] prp}) \vdash \ty{ppart}}{
							\infer[\Lex]{\ty{ppart / \dxdia[\predc]prp}}{\w{gestemd}}
							&
							\infer[\Ax]{\ty{\dxdia[\predc]prp} \vdash \ty{\dxdia[\predc]prp}}{}
						}
					}
				}
			}
...
\begin{frame}{Dependency Modalities\smaller\\
\quad\light{Semantics (?)}}\smaller
	Open question \light{\{-\,- \textit{read: no idea} -\,-\}} possible options:
	\begin{itemize}
		\item dependency marks as ``thematic roles''?
		\visible<2->{\item S4 modalities $\rightsquigarrow$ monads\\
		\visible<3->{\quad \light{these modalities $\rightsquigarrow$ ...?}}}
		\visible<4->{\item passageway to DTT proof-theoretic semantics?}
	\end{itemize}
\end{frame}

\begin{frame}{TLDR}\smaller
	Today's agenda (a posteriori)	
	\begin{itemize}
		\item MILL (from above \& below)
		\item (N)L(P) and linguistic applications
		\item Traversing between (N)L(P) with modalities
		\item Modalities for dependency demarcation
	\end{itemize}
\end{frame}

\begin{frame}{The Real World}\smaller
	\centering
	\begin{tikzpicture}[
		t/.style={text height=1.5ex, text depth=.25ex, rectangle, outer sep=0pt, color=light},
		node distance=10pt,
		...
		\node[t] (wn) 			at (4, 0.2) {w\textsubscript{n}};
		\alt<4->{
		\node[rectangle,draw=pred, minimum width=120pt,minimum height=20pt,fill=gray!20] (lex)
							 	at (2, -1.5) {\alert{Lexicon}};
		}{
		\node[rectangle,draw=black, minimum width=120pt,minimum height=20pt,fill=gray!20] (lex)
							 	at (2, -1.5) {Lexicon};
		}
		\draw[arr]  (w1) -- ++ (0.6, -1.1);
		\draw[arr] (wn) -- ++ (-0.6, -1.1);
		\node[t] (t1)			at (0, -3.2) {$\left(\ty{t}_{0,0} \mathbin{\&} \ty{t}_{0, 1} \mathbin{\&} \alt<4->{\alert{\ty{t}_{0,2}} \mathbin{\alert{\&}} \alert{\dots}}{\dots}\right)$};
		\node[t] (tdots)		at (2, -3.2) 	  {\dots};
		\node[t] (tn)			at (4, -3.2) {$\left(\ty{t}_{n,0} \mathbin{\&} \ty{t}_{n,1} \mathbin{\&} \alt<4->{\alert{\ty{t}_{n,2}} \mathbin{\alert{\&}} \alert{\dots}}{\dots}\right)$};
		\draw[arr] ($(w1) + (0.6, -2.2)$) -- ($(t1.north) + (0, 0.1)$);
		\draw[arr] ($(wn) + (-0.6, -2.2)$) -- ($(tn.north) + (0, 0.1)$);
		\draw [thick,decorate,decoration={brace,aspect=0.2,amplitude=10pt,mirror}] (-0.5,-3.5) -- (4.5,-3.5) 
				node[black,xshift=-113.5pt,yshift=-0.6cm] {\footnotesize $\times$}
				node[xshift=-140pt, yshift=-8.5em, align=right,color=light] {\smaller \alertat{5-}{$j = |\ty{t}_0| \times \dots \times |\ty{t}_n|$ judgements}};				
		\node[t] (c11)			at (1.7, -5) {$\ty{t}_{0,0} ~ \sbr$};
		\node[t] 				at (2.75, -5) {\dots};
		\node[t] (cn1)			at (4.4, -5) {$\sbr ~\ty{t}_{n,0} \vdash ?$};
		\node[t] (c12)			at (1.7, -6) {$\ty{t}_{0,0} ~ \sbr$};
		...
		\draw[arr, rounded corners] (0.5, -4.3) -- ++ (0, -3.7) -- ++ (0.65, 0);
		\alt<5->{
		\node[rectangle, draw=pred, minimum height=120pt,fill=gray!20] (pr)
								at (7.4, -6.5)	{\alert{\begin{tabular}{c}Theorem\\ Prover\end{tabular}}};	
		}{
		\node[rectangle, draw=black, minimum height=120pt,fill=gray!20] (pr)
								at (7.4, -6.5)	{\begin{tabular}{c}Theorem\\ Prover\end{tabular}};	
		}
		...
		\draw[arr] (cn2) -- ++ (1.7, 0);
		\draw[arr] (cnk) -- ++ (1.9, 0);
		\draw[arr] (cn1) ++ (4.2, 0) -- ++ (1.2, 0) node[right] {$\{p_0, p_0', \alt<2->{\alert{p_0''},}{}\dots \}$};
		\draw[arr] (cn2) ++ (4.2, 0) -- ++ (1.2, 0) node[right] {$\{p_1, \alt<3->{p_1'\Large\mathrlap{\mkern-12mu\alert{\crossmark}}}{p_1'}, \dots \}$};
		\node 					at ($(vd) + (6.6, 0)$) {\vdots};
		\draw[arr] (cnk) ++ (4.4, 0) -- ++ (1.2, 0) node[right] {$\{p_j, p_j', \dots \}$};
		...
		\node[tb]				at (6, -0.25) {	
				\begin{itemize}
					\item[\textbf{!}] \eqmakebox[things][l]{logic too strict}
