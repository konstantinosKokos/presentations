...
\begin{frame}{(2) The Neural Lexicon \visible<7->{-- \light{take \# 2}}}
	\centering
	...
	\node[t] (ss)			at (0.4, 0.40)	{\alt<11,16>{\textcolor{red}{/}}{/}};
	\node[t] (bs)			at (-0.4, 0.40)	{\alt<17,21>{\textcolor{red}{$\backslash$}}{$\backslash$}};
	...
	\visible<-6>	{
	\node[t]	 (np)			at (-0.5, -0.6)	{\alt<3,5>{\textcolor{red}{$np$}}{$np$}};
	\node[t]	 (mod)			at (1.9, -1.3) 	{\alt<4>{\textcolor{red}{$\Box^{mod}(np/np)$}}{$\Box^{mod}(np/np)$}};
	\node[t] (v)				at (0.9, 0.5) 	{$(\diamondsuit^{su}np \backslash s)/\diamondsuit^{obj}np$};
	\draw[c] plot [smooth cycle, tension=1] coordinates {(1.5, 1) (-1.2, -0.2) (1.5, -1.8) (3.2, -0.8)};
	}
	...
	\visible<11,16>{
	\draw[->, thick, red] (lex) -- (ss);
	}
	\visible<17,21>{
	\draw[->, thick, red] (lex) -- (bs);
	}
	\visible<18,22>{
	...
	\visible<15->{
	\node[t, text width=140pt] (vpmod)							at (4, -2.5)		{\textcolor{red}{
		$\Box^{mod}\left(
			\alt<16->{
				\alt<17->{
					\left(
...
\begin{frame}{(4) Neural Proof Nets}	
	\centering
	\alt<6->{
		\begin{tikzpicture}
...
\begin{frame}{Just out of time \light{(hopefully)}}
		\begin{itemize}
			...
			\uncover<2->{
			\item Dependency as Modality, Parsing as Permutation\\
			\light{github.com/konstantinosKokos/phd-thesis}\\
			\uncover<3->{
			\item this presentation\\
			\light{github.com/konstantinosKokos/presentations}
			}}
		\end{itemize}\vfill
